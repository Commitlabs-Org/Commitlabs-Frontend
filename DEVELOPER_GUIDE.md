...
The project uses [Prettier](https://prettier.io/) as its code formatter to ensure consistent style across all files.
...
**Ignored files**: see `.prettierignore` (build output, dependencies, lockfiles, etc.)
...
# Check formatting without making changes (CI gate)
pnpm format:check
```

Formatting is checked in CI via the `format:check` script. The pre-commit hook also ensures all staged files are properly formatted.
...
4.  **Format & Lint**: Run `pnpm format` and `pnpm lint` to check for errors.
