 ...
 */
function getPenaltyPercent(commitment: Record<string, any>, tier: PenaltyTier): number {
  ...
  const scaledPenaltyPercent = basePenaltyPercent * (remainingDurationMs / totalDurationMs);

  return +Math.max(0, Math.min(basePenaltyPercent, scaledPenaltyPercent)).toFixed(4);
}
...
export const GET = withApiHandler(async (req: NextRequest, { params }) => {
  ...
  if (commitment.status !== 'ACTIVE') {
    throw new ConflictError('Commitment must be active to preview an early exit');
  }
  ...
  const penaltyPercent = getPenaltyPercent(commitment as Record<string, any>, tier);
