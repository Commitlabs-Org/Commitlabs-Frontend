      ...
      <div className="flex items-center gap-4 min-w-[150px]">
        ...
        <div>
          <div className="flex items-center gap-2">
            <div
              className={`inline-flex rounded-full px-2.5 py-1 sm:py-0.5 text-[10px] font-bold uppercase tracking-wider ${badgeClass}`}
            >
              {item.type}
            </div>
            <TrustBadge level={item.trustLevel ?? 'unverified'} showTooltip={false} />
          </div>
          <div className="font-mono text-sm text-white/50 mt-1 sm:mt-0">
            #CMT-{item.id.padStart(3, '0')}
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col min-w-[120px] col-span-2 sm:col-span-1">
          ...
          <span className="text-lg font-bold">
            {item.forSale ? (
              ...
              <span className="text-white/30 text-base font-normal">Not for sale</span>
            )}
          </span>
        </div>
      </div>
  ...
  return (
    <AppShellLayout>
      <div className="min-h-screen w-full bg-[#0a0a0a] text-white overflow-x-hidden">
        <main
          id="main-content"
          className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 pt-8 md:pt-12 pb-10 relative"
        >
          ...
          `}
            ...
            <div className="flex-1 min-w-0 w-full">
              {isLoading ? (
                <MarketplaceGridSkeleton showFilters={false} cardCount={9} />
              ) : (
