...
describe('error pages documentation drift guard', () => {
  ...
  it('keeps the documented recovery CTA labels in sync with the actual components', () => {
    ...
    expectSourceContainsAll(networkErrorText, [
      'Retry',
      'Go Home',
      'Checking connection...',
      'Retrying...',
    ]);
    expectSourceContainsAll(transactionErrorText, [
      'Try Again',
      'Go to Dashboard',
      'Check Explorer',
      'View on Explorer',
  ...
  it('covers the helper branches used for route discovery and file validation', () => {
    ...
    expect(normalizeText(readWorkspaceFile('ERROR_PAGES_README.md'))).toContain(
