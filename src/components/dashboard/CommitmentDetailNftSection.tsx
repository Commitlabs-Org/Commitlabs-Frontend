    ...
    <section className="w-full space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-[#0F1115] border border-white/5">
        ...
        <div className="space-y-4">
          <MetadataRow label="Token ID" value={tokenId} onCopy={onCopyTokenId} />
          <MetadataRow label="Owner" value={ownerAddress} isAddress onCopy={onCopyOwner} />
          <MetadataRow
  ...
  <div className="flex justify-between items-center py-2 border-b border-white/5">
    ...
    <div className="flex items-center gap-2">
      <span className={`text-sm font-medium text-white ${isAddress ? 'font-mono' : ''}`}>
        ...
        {isAddress && value.length > 12 ? `${value.slice(0, 6)}...${value.slice(-4)}` : value}
      </span>
      <button
        onClick={onCopy}
