...
function getSocialIconsContainer(): HTMLElement {
  ...
  return within(bottomBar).getByRole('link', { name: 'Twitter' }).parentElement as HTMLElement;
}
...
describe('Footer', () => {
  ...
  describe('external link safety attributes', () => {
    it('does NOT add target/rel safety attributes to the external GitHub link in the Resources group', () => {
      ...
      expect(githubLink).toHaveAttribute('href', 'https://github.com/commitlabs');
    ...
    it('points each social icon link at its real external href', () => {
      ...
      expect(within(socialIcons).getByRole('link', { name: 'Twitter' })).toHaveAttribute(
        'href',
        'https://twitter.com/commitlabs',
      );
      expect(within(socialIcons).getByRole('link', { name: 'GitHub' })).toHaveAttribute(
        'href',
  ...
  describe('social icons', () => {
    it('renders exactly three social icon links: Twitter, GitHub, Forum', () => {
      ...
      socialIconLabels.forEach((label) => {
        expect(within(socialIcons).getByRole('link', { name: label })).toBeInTheDocument();
      });
    });
  ...
  describe('branding and legal content', () => {
    ...
    it('renders the copyright / license line', () => {
      ...
      expect(screen.getByText(/© 2026 CommitLabs\. Licensed under MIT\./i)).toBeInTheDocument();
    });
  });
});
