      ...
      <ul className="flex flex-col gap-3 p-0 m-0 list-none">
        {constraints.map((constraint) => (
          ...
          >
            ...
            <span className="text-[15px] text-[#e0e0e0] font-medium">{constraint.text}</span>
          </li>
        ))}
      </ul>
      ...
      <div className="mt-6 rounded-xl border border-white/[0.08] bg-[#0A0A0B] p-5 relative overflow-hidden">
        ...
        <div className="relative flex gap-3 z-10 items-start">
          ...
          <p className="text-sm text-[#99a1af] leading-relaxed">
            ...
            {noteText
              .replace(/^On-chain enforcement:\s*/i, '')
              .split(/(attestations?)/i)
              .map((part, i) =>
                /attestations?/i.test(part) ? (
                  <GlossaryTerm key={i} termKey="attestation">
                    {part}
                  </GlossaryTerm>
                ) : (
                  part
