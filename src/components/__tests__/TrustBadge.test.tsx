...
describe('TrustBadge', () => {
  ...
  it.each(TRUST_LEVELS)(
    ...
    ({ level, label, description }) => {
      ...
      expect(badge).toHaveAttribute('aria-describedby', `trust-badge-tooltip-${level}`);
  ...
  it('does not render tooltip content when showTooltip is false', () => {
    ...
    expect(screen.getByRole('status', { name: 'Verified Seller' })).not.toHaveAttribute(
      'aria-describedby',
    );
  });

  it('merges custom className with default badge styles', () => {
    render(<TrustBadge level="verified" className="custom-trust-badge" showTooltip={false} />);
