...
describe('MarketplaceCard', () => {
  ...
  describe('type variants', () => {
    it.each(TYPE_FIXTURES)('renders $type badge label', ({ type }) => {
      renderCard({ type });
      expect(screen.getByText(type)).toBeInTheDocument();
    });

    it.each(TYPE_FIXTURES)('applies correct border class for $type', ({ type, borderClass }) => {
      renderCard({ type });
      expect(getArticle().className).toContain(borderClass);
    });

    it.each(TYPE_FIXTURES)('applies correct badge background for $type', ({ type, badgeClass }) => {
      renderCard({ type });
      const badge = screen.getByText(type);
      expect(badge.className).toContain(badgeClass);
    });

    it.each(TYPE_FIXTURES)('applies correct score color for $type', ({ type, scoreClass }) => {
    ...
    it.each(TYPE_FIXTURES)('renders an SVG type icon for $type', ({ type }) => {
  ...
  describe('forSale=false', () => {
    ...
    it('opens the detail modal when View is clicked', () => {
      ...
      expect(screen.getByTestId('commitment-modal')).toBeInTheDocument();
      expect(screen.getByTestId('commitment-modal')).toHaveAttribute('data-commitment-id', '7');
    });
  });
  ...
  describe('accessible semantics', () => {
    it("article has aria-label 'Commitment {id}'", () => {
      renderCard({ id: '42' });
      expect(screen.getByRole('article', { name: 'Commitment 42' })).toBeInTheDocument();
    });
