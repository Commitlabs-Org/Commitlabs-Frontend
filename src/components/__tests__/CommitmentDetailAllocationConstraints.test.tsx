...
describe('CommitmentDetailAllocationConstraints', () => {
  it('renders headroom gauge with correct aria values', () => {
    render(<CommitmentDetailAllocationConstraints constraints={[]} commitment={mockCommitment} />);
    ...
    expect(
      screen.getByText(`${mockCommitment.currentDrawdown} / ${mockCommitment.maxLoss}`),
    ).toBeInTheDocument();
  });
});
