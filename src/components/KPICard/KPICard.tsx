...
import { TrendingUp, TrendingDown, Minus, AlertCircle, Loader2, LucideIcon } from 'lucide-react';
 ...
 */
export function calculateDelta(current: string | number, previous: string | number): KPIDelta {
...
const ErrorState: React.FC<ErrorStateProps> = ({ message, onRetry, size = 'medium' }) => {
  return (
    <div
      className={clsx(
        styles.errorState,
        styles[`error${size.charAt(0).toUpperCase() + size.slice(1)}`],
      )}
    >
      <AlertCircle
...
const EmptyState: React.FC<EmptyStateProps> = ({ message, size = 'medium' }) => {
  return (
    <div
      className={clsx(
        styles.emptyState,
        styles[`empty${size.charAt(0).toUpperCase() + size.slice(1)}`],
      )}
    >
...
export const KPICard: React.FC<KPICardProps> = ({
  ...
  return (
    <div
      className={clsx(styles.card, styles[variant], styles[size], {
    ...
    >
      ...
      <div className={styles.valueContainer}>
        ...
        {displayDelta && <DeltaIndicator delta={displayDelta} size={size} />}
      </div>
