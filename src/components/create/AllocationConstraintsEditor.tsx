...
export default function AllocationConstraintsEditor({
  ...
  const typeLabel =
    commitmentType === 'safe' ? 'Safe' : commitmentType === 'balanced' ? 'Balanced' : 'Aggressive';
  ...
  const gaugeColor =
    maxLossPercent <= 30 ? '#00d4aa' : maxLossPercent <= 70 ? '#f5a623' : '#ff4444';
      ...
      <div className="rounded-xl border border-white/[0.08] bg-[#0A0A0B] p-4 relative overflow-hidden">
        ...
        <div className="relative flex gap-3 z-10 items-start">
          ...
          <p className="text-sm text-[#99a1af] leading-relaxed">
            ...
            Constraints are enforced by smart contracts and cannot be changed after commitment
            creation. Violations are automatically detected and recorded as attestations.
          </p>
        </div>
      </div>
    </section>
