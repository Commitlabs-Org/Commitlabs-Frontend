...
describe('parseDecimalString', () => {
  ...
  describe('invalid inputs', () => {
    ...
    it('rejects too many decimal places', () => {
      expect(() => parseDecimalString('123.12345678')).toThrow(BadRequestError);
    });
  ...
  describe('bounds validation', () => {
    ...
    it('respects max option', () => {
      ...
      expect(() => parseDecimalString('150', { max: 100 })).toThrow(BadRequestError);
    });

    it('respects min and max together', () => {
      expect(parseDecimalString('50', { min: 0, max: 100 })).toBe(50);
      expect(() => parseDecimalString('-10', { min: 0, max: 100 })).toThrow(BadRequestError);
    });

    it('respects allowNegative option', () => {
      expect(parseDecimalString('-10', { allowNegative: true })).toBe(-10);
      expect(() => parseDecimalString('-10', { allowNegative: false })).toThrow(BadRequestError);
    });
...
describe('parseBps', () => {
  ...
  describe('options', () => {
    it('respects custom min option', () => {
      ...
      expect(() => parseBps(50, { min: 100 })).toThrow(BadRequestError);
    });

    it('respects custom max option', () => {
      expect(parseBps(5000, { max: 10000 })).toBe(5000);
      expect(() => parseBps(15000, { max: 10000 })).toThrow(BadRequestError);
    });
...
describe('parsePositiveInt', () => {
  ...
  describe('invalid inputs', () => {
    ...
    it('rejects unsafe integers', () => {
      ...
      expect(() => parsePositiveInt(String(unsafe))).toThrow(BadRequestError);
    });
  ...
  describe('options', () => {
    it('respects max option', () => {
      ...
      expect(() => parsePositiveInt(150, { max: 100 })).toThrow(BadRequestError);
    });
