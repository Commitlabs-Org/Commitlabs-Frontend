...
export function parseDecimalString(value: unknown, options: ParseDecimalOptions = {}): number {
  ...
  if (!decimalRegex.test(trimmed)) {
    throw new BadRequestError('Invalid decimal format. Expected a numeric string.');
  }

  if (trimmed.includes('.')) {
    const decimals = trimmed.split('.')[1]!.length;
    if (decimals > 7) {
      throw new BadRequestError('Value has too many decimal places. Maximum 7 allowed.');
    }
  }
...
function applyBounds(value: number, options: ParseDecimalOptions): number {
  if (options.min !== undefined && value < options.min) {
    throw new BadRequestError(`Value must be at least ${options.min}`);
  }
...
export function parsePositiveInt(value: unknown, options: ParsePositiveIntOptions = {}): number {
  if (value === null || value === undefined) {
    throw new BadRequestError('Value is required');
  }
...
export function parseAmountWithBounds(value: unknown, min: number, max: number): number {
  return parseDecimalString(value, {
    min,
    max,
    allowNegative: false,
