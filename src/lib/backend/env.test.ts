...
import { validateEnv, getValidatedEnv, _resetEnvCache, EnvValidationError } from './env';
...
describe('EnvValidationError', () => {
  ...
  it('includes path and message in the error string', () => {
    const err = new EnvValidationError([{ path: 'SOME_VAR', message: 'is required' }]);
...
describe('validateEnv — development environment', () => {
  ...
  it('passes SESSION_SECRET that meets the 32-char minimum', () => {
    expect(() => validateEnv({ ...DEV_ENV, SESSION_SECRET: 'a'.repeat(32) })).not.toThrow();
  });
  ...
  it('ignores unknown env vars (they are stripped by Zod)', () => {
    expect(() => validateEnv({ ...DEV_ENV, UNKNOWN_VAR: 'anything' })).not.toThrow();
  });
});
...
describe('validateEnv — URL validation', () => {
  it('rejects an invalid SOROBAN_RPC_URL', () => {
    expect(() => validateEnv({ ...DEV_ENV, SOROBAN_RPC_URL: 'not-a-url' })).toThrow(
      EnvValidationError,
    );
  });
  ...
  it('includes the field name in the error for an invalid SOROBAN_RPC_URL', () => {
    try {
      ...
      const issue = (err as EnvValidationError).issues.find((i) => i.path === 'SOROBAN_RPC_URL');
      expect(issue).toBeDefined();
    }
  });

  it('rejects an invalid NEXT_PUBLIC_SOROBAN_RPC_URL', () => {
    expect(() => validateEnv({ ...DEV_ENV, NEXT_PUBLIC_SOROBAN_RPC_URL: 'no-protocol' })).toThrow(
      EnvValidationError,
    );
  });
...
describe('validateEnv — sensitive field redaction', () => {
  ...
  it('redacts SESSION_SECRET in production missing-field error', () => {
    try {
      ...
      expect(err).toBeInstanceOf(EnvValidationError);
      const issue = (err as EnvValidationError).issues.find((i) => i.path === 'SESSION_SECRET');
      expect(issue?.message).toContain('value redacted');
    }
  });
...
describe('getValidatedEnv', () => {
  ...
  it('throws EnvValidationError when the source fails validation', () => {
    expect(() => getValidatedEnv({ NODE_ENV: 'production' })).toThrow(EnvValidationError);
  });
