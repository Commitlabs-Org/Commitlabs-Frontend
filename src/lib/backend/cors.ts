...
function getConfiguredFirstPartyOrigins(): string[] {
  ...
  if (configured === '*') {
    throw new Error(
      'COMMITLABS_FIRST_PARTY_ORIGINS cannot be "*" because first-party routes may allow credentials.',
    );
  }
...
function getAllowedOrigins(access: CorsAccess): AllowedOrigins {
  return access === 'first-party' ? getConfiguredFirstPartyOrigins() : getConfiguredPublicOrigins();
}
...
function validateRequestedHeaders(request: NextRequest, allowedHeaders: string[]): string[] {
  ...
  const invalidHeader = requestedHeaders.find((header) => !allowedLookup.has(header.toLowerCase()));
...
export function createCorsOptionsHandler(routePolicy: CorsRoutePolicy) {
  return async function OPTIONS(request: NextRequest): Promise<NextResponse> {
    try {
      ...
      const allowedHeaders = validateRequestedHeaders(request, getAllowedHeaders(resolved.policy));
