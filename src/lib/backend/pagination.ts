...
export function parsePaginationParams(
  ...
  const page = rawPage !== null ? parsePositiveInt(rawPage, 'page') : DEFAULT_PAGE;
  const pageSize =
    rawPageSize !== null
...
export function parseRangeFilter(
  ...
  if (min !== undefined && options.min !== undefined && min < options.min) {
    errors.push({
      param: `${paramName}Min`,
      message: `'${paramName}Min' must be at least ${options.min}.`,
    });
  }
  if (max !== undefined && options.max !== undefined && max > options.max) {
    errors.push({
      param: `${paramName}Max`,
      message: `'${paramName}Max' must be at most ${options.max}.`,
    });
  }
...
function parsePositiveInt(raw: string, paramName: string, options: { max?: number } = {}): number {
