...
describe('parseJsonWithLimit', () => {
  ...
  it('honours an explicit limitBytes option', async () => {
    ...
    await expect(parseJsonWithLimit(req, { limitBytes: 1024 })).resolves.toEqual(payload);
  });
  ...
  it('throws PayloadTooLargeError when Content-Length advertises oversized body', async () => {
    ...
    await expect(parseJsonWithLimit(req)).rejects.toBeInstanceOf(PayloadTooLargeError);
  });
  ...
  it('accepts JSON that is exactly at the configured limit', async () => {
    ...
    await expect(parseJsonWithLimit(req, { limitBytes: limit })).resolves.toEqual({
      d: 'a'.repeat(padLength),
    });
  });
  ...
  describe.each([
    ...
    it('throws PayloadTooLargeError when body is one byte over the limit', async () => {
      ...
      const err = await parseJsonWithLimit(req, { limitBytes: limit }).catch((e) => e);
