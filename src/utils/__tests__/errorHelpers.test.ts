...
describe('rateLimitError', () => {
  it('returns code 429 and correct type and message', () => {
    ...
    expect(result.error.message).toBe('Too many requests. Please wait before trying again.');
  });
...
describe('gatewayTimeoutError', () => {
  it('returns code 504 and correct type and message', () => {
    ...
    expect(result.error.message).toBe('The request timed out. Please try again.');
  });
...
describe('resolveServerError', () => {
  ...
  it.each([
    ...
    [599, 'INTERNAL_SERVER_ERROR', 'An unexpected error occurred. Please try again later.'],
  ])('maps status %i to the expected user-facing %s message', (statusCode, errorType, message) => {
