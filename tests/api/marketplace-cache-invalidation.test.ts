...
describe('Marketplace Cache Invalidation', () => {
  ...
  describe('Listing queries cache', () => {
    ...
    it('evicts listing query cache entries via prefix invalidation', async () => {
      ...
      await adapter.set(key2, [mockListing, mockListing], CacheTTL.MARKETPLACE_LISTINGS);
  ...
  describe('Cache isolation', () => {
    it('does not evict stats cache when invalidating listings prefix', async () => {
      ...
      await adapter.set(listingKey, [mockListing], CacheTTL.MARKETPLACE_LISTINGS);
      await adapter.set(statsKey, mockStats, CacheTTL.MARKETPLACE_STATS);
    ...
    it('does not invalidate other cache namespaces', async () => {
      ...
      await adapter.set(commitmentKey, mockCommitment, CacheTTL.COMMITMENT_DETAIL);
      await adapter.set(listingKey, [mockListing], CacheTTL.MARKETPLACE_LISTINGS);
  ...
  describe('Invalidation triggering scenarios', () => {
    it('simulates listing creation invalidating all queries and stats', async () => {
      ...
      // Pre-populate caches
      await adapter.set(queryKey1, [mockListing], CacheTTL.MARKETPLACE_LISTINGS);
      await adapter.set(queryKey2, [mockListing], CacheTTL.MARKETPLACE_LISTINGS);
      await adapter.set(statsKey, mockStats, CacheTTL.MARKETPLACE_STATS);
    ...
    it('handles multiple consecutive invalidations safely', async () => {
      ...
      await adapter.set(queryKey, [mockListing, mockListing], CacheTTL.MARKETPLACE_LISTINGS);
      await adapter.set(
        statsKey,
        {
          activeListings: 2,
  ...
  describe('Edge cases', () => {
    ...
    it('preserves cache isolation when different prefixes are invalidated', async () => {
      ...
      await adapter.set(listingKey, [mockListing], CacheTTL.MARKETPLACE_LISTINGS);
      await adapter.set(commitmentKey1, { id: 'CMT-001' }, CacheTTL.COMMITMENT_DETAIL);
      await adapter.set(commitmentKey2, { id: 'CMT-002' }, CacheTTL.COMMITMENT_DETAIL);
