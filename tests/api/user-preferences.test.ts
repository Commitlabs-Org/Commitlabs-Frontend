...
import { GET, PUT, __setStoreForTesting, __resetStore } from '@/app/api/user/preferences/route';
import {
  DEFAULT_PREFERENCES,
...
describe('PUT /api/user/preferences', () => {
  ...
  it('returns 401 when token format is invalid', async () => {
    const res = await PUT(putReq({ displayCurrency: 'EUR' }, { authorization: 'Bearer garbage' }), {
      params: {},
    });
    const { status, data } = await parseResponse(res);
