...
describe('CommitmentEarlyExitModal', () => {
  ...
  it('renders nothing when isOpen is false', () => {
    const { container } = render(<CommitmentEarlyExitModal {...defaultProps} isOpen={false} />);
  ...
  it('renders the modal header and informational notice when open', () => {
    ...
    expect(
      screen.getByText('This action is irreversible and carries penalties.'),
    ).toBeInTheDocument();

    // Notice check
    expect(screen.getByText('Important consequences')).toBeInTheDocument();
    expect(
      screen.getByText('You will lose the penalty amount shown above immediately.'),
    ).toBeInTheDocument();
  });
  ...
  describe('accessibility and semantic markup', () => {
    ...
    it('applies accessible screen reader labels spelling out units and currency codes', () => {
      ...
      const penaltyAmountCell = screen.getByLabelText(
        'Penalty deduction: minus 1,000 Stellar Lumens',
      );
  ...
  describe('user interactions and confirmation validation', () => {
    ...
    it('triggers onChangeAcknowledged callback when clicking acknowledgment checkbox', () => {
      ...
      render(
        <CommitmentEarlyExitModal {...defaultProps} onChangeAcknowledged={onChangeAcknowledged} />,
      );
    ...
    it('calls onCancel or onClose when appropriate buttons are clicked', () => {
      ...
      render(<CommitmentEarlyExitModal {...defaultProps} onCancel={onCancel} onClose={onClose} />);
    ...
    it('calls onConfirm when confirm button is clicked', () => {
      ...
      render(
        <CommitmentEarlyExitModal {...defaultProps} hasAcknowledged={true} onConfirm={onConfirm} />,
      );
  ...
  describe('penalty preview calculations per risk tier', () => {
    it('asserts correct rendering and labels for Safe (2%) tier calculations', () => {
      ...
      // Value text matchers
      expect(screen.getByLabelText('Committed amount: 50,000 Stellar Lumens')).toHaveTextContent(
        '50,000 XLM',
      );
      expect(screen.getByLabelText('Penalty rate: 2 percent')).toHaveTextContent('2%');
      expect(
        screen.getByLabelText('Penalty deduction: minus 1,000 Stellar Lumens'),
      ).toHaveTextContent('-1,000 XLM');
      expect(screen.getByLabelText('Net refund amount: 49,000 Stellar Lumens')).toHaveTextContent(
        '49,000 XLM',
      );
    });
    ...
    it('asserts correct rendering and labels for Balanced (3%) tier calculations', () => {
      ...
      // Value text matchers
      expect(screen.getByLabelText('Committed amount: 100,000 USD Coin')).toHaveTextContent(
        '100,000 USDC',
      );
      expect(screen.getByLabelText('Penalty rate: 3 percent')).toHaveTextContent('3%');
      expect(screen.getByLabelText('Penalty deduction: minus 3,000 USD Coin')).toHaveTextContent(
        '-3,000 USDC',
      );
      expect(screen.getByLabelText('Net refund amount: 97,000 USD Coin')).toHaveTextContent(
        '97,000 USDC',
      );
    });
    ...
    it('asserts correct rendering and labels for Aggressive (5%) tier calculations', () => {
      ...
      // Value text matchers
      expect(screen.getByLabelText('Committed amount: 250,000 Stellar Lumens')).toHaveTextContent(
        '250,000 XLM',
      );
      expect(screen.getByLabelText('Penalty rate: 5 percent')).toHaveTextContent('5%');
      expect(
        screen.getByLabelText('Penalty deduction: minus 12,500 Stellar Lumens'),
      ).toHaveTextContent('-12,500 XLM');
      expect(screen.getByLabelText('Net refund amount: 237,500 Stellar Lumens')).toHaveTextContent(
        '237,500 XLM',
      );
    });
  });
