...
const DummyTypeIcon = ({ type }: { type: 'Safe' | 'Balanced' | 'Aggressive' }) => (
...
describe('CommitmentDetailsModal', () => {
  it('renders a scannable quick-view with a full details link', () => {
    ...
    expect(screen.getByRole('link', { name: /view full details/i })).toHaveAttribute(
      'href',
      '/commitments/commitment-123',
    );
  });

  it('closes when Escape is pressed and cycles focus inside the modal', () => {
    const onClose = vi.fn();
    render(<CommitmentDetailsModal {...defaultProps} isOpen={true} onClose={onClose} />);
  ...
  it('restores focus to the previously focused element when the modal closes', () => {
    ...
    const { rerender } = render(
      <CommitmentDetailsModal {...defaultProps} isOpen={true} onClose={onClose} />,
    );

    rerender(<CommitmentDetailsModal {...defaultProps} isOpen={false} onClose={onClose} />);
