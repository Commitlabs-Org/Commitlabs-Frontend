...
vi.mock('@stellar/stellar-sdk', () => {
  function FakeAccount(this: { publicKey: string; sequence: string }, pk: string, seq: string) {
    this.publicKey = pk;
    this.sequence = seq;
  }
  ...
  return {
    Account: FakeAccount,
    Address: function FakeAddress(this: { addr: string; toScVal: () => string }, addr: string) {
      this.addr = addr;
      this.toScVal = () => addr;
    },
    BASE_FEE: 100,
...
describe('Soroban RPC abort/timeout handling', () => {
  ...
  it('maps a hanging simulateTransaction to GATEWAY_TIMEOUT on read', async () => {
    ...
    const settled = getCommitmentFromChain('commitment-read-hang').catch((e: unknown) => e);
    ...
    expect(((err as BackendError).details as Record<string, unknown>).retryable).toBe(true);
  });
  ...
  it('includes methodName and timeoutMs in timeout error details', async () => {
    ...
    const settled = getCommitmentFromChain('cid-detail-check').catch((e: unknown) => e);
    await vi.advanceTimersByTimeAsync(600);
    const err = await settled;
