...
describe('validateCommitmentDraft', () => {
  ...
  describe('ownerAddress StrKey validation', () => {
    ...
    it('rejects address with bad prefix', () => {
      const result = validateCommitmentDraft(
        validDraft({ ownerAddress: INVALID_ADDRESS_BAD_PREFIX }),
      );
      expect(result.valid).toBe(false);
      expect(result.errors[0].field).toBe('ownerAddress');
    });
...
describe('validateFilters', () => {
  ...
  it('skips undefined values', () => {
    expect(validateFilters({ maxPrice: undefined, category: 'test' })).toEqual({
      category: 'test',
    });
  });
...
describe('createCommitmentSchema', () => {
  ...
  it('rejects unsupported asset', () => {
    expect(() => createCommitmentSchema.parse({ ...validInput, asset: UNSUPPORTED_ASSET })).toThrow(
      z.ZodError,
    );
  });
  ...
  it('rejects zero amount', () => {
    expect(() => createCommitmentSchema.parse({ ...validInput, amount: 0 })).toThrow(z.ZodError);
  });

  it('rejects negative amount', () => {
    expect(() => createCommitmentSchema.parse({ ...validInput, amount: -1 })).toThrow(z.ZodError);
  });

  it('rejects durationDays below 1', () => {
    expect(() => createCommitmentSchema.parse({ ...validInput, durationDays: 0 })).toThrow(
      z.ZodError,
    );
  });

  it('rejects durationDays above 365', () => {
    expect(() => createCommitmentSchema.parse({ ...validInput, durationDays: 366 })).toThrow(
      z.ZodError,
    );
  });

  it('rejects negative maxLossBps', () => {
    expect(() => createCommitmentSchema.parse({ ...validInput, maxLossBps: -1 })).toThrow(
      z.ZodError,
    );
  });
...
describe('createMarketplaceListingSchema', () => {
  ...
  it('rejects empty title', () => {
    expect(() => createMarketplaceListingSchema.parse({ ...validInput, title: '' })).toThrow(
      z.ZodError,
    );
  });

  it('rejects empty category', () => {
    expect(() => createMarketplaceListingSchema.parse({ ...validInput, category: '' })).toThrow(
      z.ZodError,
    );
  });

  it('rejects zero price', () => {
    expect(() => createMarketplaceListingSchema.parse({ ...validInput, price: 0 })).toThrow(
      z.ZodError,
    );
  });

  it('rejects negative price', () => {
    expect(() => createMarketplaceListingSchema.parse({ ...validInput, price: -10 })).toThrow(
      z.ZodError,
    );
  });
...
describe('createAttestationSchema', () => {
  ...
  it('rejects empty commitmentId', () => {
    expect(() => createAttestationSchema.parse({ ...validInput, commitmentId: '' })).toThrow(
      z.ZodError,
    );
  });
  ...
  it('rejects rating below 1', () => {
    expect(() => createAttestationSchema.parse({ ...validInput, rating: 0 })).toThrow(z.ZodError);
  });

  it('rejects rating above 5', () => {
    expect(() => createAttestationSchema.parse({ ...validInput, rating: 6 })).toThrow(z.ZodError);
  });

  it('rejects non-integer rating', () => {
    expect(() => createAttestationSchema.parse({ ...validInput, rating: 3.5 })).toThrow(z.ZodError);
  });
});
...
describe('ResolveDisputeSchema', () => {
  ...
  it('rejects invalid resolution value', () => {
    expect(() => ResolveDisputeSchema.parse({ resolution: 'invalid_value' })).toThrow(z.ZodError);
  });
