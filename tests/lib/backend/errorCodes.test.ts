...
describe('ERROR_CODE_REGISTRY', () => {
  ...
  describe('HTTP Status Code Mapping', () => {
    ...
    it('should map 429 codes correctly', () => {
      const fourTwentySwitchNineCodes = Object.values(ERROR_CODE_REGISTRY).filter(
        (def) => def.statusCode === 429,
      );
  ...
  describe('validateErrorCodeRegistry()', () => {
    ...
    it('should validate structure in detail', () => {
      ...
      expect(validation.errors.filter((e) => e.includes("'code'"))).toHaveLength(0);
      expect(validation.errors.filter((e) => e.includes("'meaning'"))).toHaveLength(0);
      expect(validation.errors.filter((e) => e.includes("'clientHandling'"))).toHaveLength(0);
      expect(validation.errors.filter((e) => e.includes("'description'"))).toHaveLength(0);
    });
  });
  ...
  describe('getErrorCodesByStatus()', () => {
    ...
    it('should group 400 codes together', () => {
      ...
      expect(fourHundredCodes).toContainEqual(expect.objectContaining({ code: 'BAD_REQUEST' }));
      expect(fourHundredCodes).toContainEqual(
  ...
  describe('Documentation Completeness', () => {
    ...
    it('should have description that starts with trigger condition', () => {
      Object.entries(ERROR_CODE_REGISTRY).forEach(([code, def]) => {
        expect(def.description.toLowerCase()).toMatch(/triggered|thrown|returned/);
      });
    });
  });
