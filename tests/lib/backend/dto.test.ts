...
function commitmentModel(overrides: Partial<ChainCommitmentModel> = {}): ChainCommitmentModel {
  return {
    id: 'cmt-001',
...
function attestationModel(overrides: Partial<ChainAttestationModel> = {}): ChainAttestationModel {
  return {
    id: 'att-001',
...
describe('mapCommitmentFromChain', () => {
  ...
  describe('null / undefined handling', () => {
    ...
    it('sets assetIssuer to null when assetCode is XLM', () => {
      const result = mapCommitmentFromChain(
        commitmentModel({ assetCode: 'XLM', assetIssuer: 'GB123...' }),
      );
...
describe('mapAttestationFromChain', () => {
  ...
  describe('observedAt (toIsoDate) handling', () => {
    it('uses provided ISO string', () => {
      const result = mapAttestationFromChain(
        attestationModel({ observedAt: '2025-01-15T08:30:00.000Z' }),
      );
