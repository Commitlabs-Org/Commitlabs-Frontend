...
describe('redact', () => {
  ...
  it('honors case-sensitive matching when requested', () => {
    expect(redact({ token: 'redact', TOKEN: 'preserve' }, { caseInsensitive: false })).toEqual({
      token: '[REDACTED]',
      TOKEN: 'preserve',
    });
  });
...
describe('redaction helpers', () => {
  ...
  it('creates a redacted object with caller-provided sensitive fields', () => {
    expect(createRedacted({ email: 'alice@example.com', displayName: 'Alice' }, ['email'])).toEqual(
      { email: '[REDACTED]', displayName: 'Alice' },
    );
  });
