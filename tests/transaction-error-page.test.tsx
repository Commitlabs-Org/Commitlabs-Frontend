...
describe('transaction error page recovery actions', () => {
  ...
  it('renders rejected transaction recovery from normalized validation errors', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 2, name: 'No transaction was completed' }),
    ).toBeInTheDocument();
    expect(
      screen.getByText('Confirm the wallet signature prompt was approved.'),
    ).toBeInTheDocument();
  ...
  it('renders timeout recovery with an explorer check before retry guidance', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 1, name: 'Transaction Status Unknown' }),
    ).toHaveFocus();
    expect(
      screen.getByRole('heading', { level: 2, name: 'Check before resubmitting' }),
    ).toBeInTheDocument();
    expect(
      screen.getByText('Avoid submitting the same signed transaction twice.'),
    ).toBeInTheDocument();
  ...
  it('renders failed transaction recovery for normalized chain call failures', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 2, name: 'The action did not finish' }),
    ).toBeInTheDocument();
    expect(
      screen.getByText('Check whether the smart contract rejected the operation.'),
    ).toBeInTheDocument();
